...
def convert_svo_to_rgb_pc(filepath, recording_folderpath, pc_folderpath):
    # Open SVO Reader #
    serial_number = filepath.split("/")[-1][:-5]
    camera = SVOReader(filepath, serial_number=serial_number)
    camera.set_reading_parameters(image=True, depth=False, pointcloud=True, concatenate_images=True)
    ...
    pc_output_path = os.path.join(pc_folderpath, serial_number + ".h5")
    ...
    for _i in range(frame_count):
        ...
        pc_frame = data_dict['pointcloud'][f'{serial_number}_left']
        pc_arr.append(pc_frame[np.newaxis,:])
    ...
    pc_arr = np.concatenate(pc_arr, axis=0)

    # # save pc array to h5 file
    with h5py.File(pc_output_path, 'w') as file:  
        file.create_dataset(pc_output_path, data=pc_arr)  
...
# add time tag
data_dir = os.path.join(data_dir,"2024-08-14")

# import pdb; pdb.set_trace()

all_folderpaths = all_folderpaths = crawler(data_dir)
for folderpath in tqdm(all_folderpaths):
    ...
    pc_folderpath = os.path.join(recording_folderpath, "PCD")
    ...
    if not os.path.exists(pc_folderpath):
        os.makedirs(pc_folderpath)
    ...
    for f in files_to_convert:
        convert_svo_to_rgb_pc(f, recording_folderpath, pc_folderpath)
