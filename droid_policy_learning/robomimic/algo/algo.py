...
def algo_factory(algo_name, config, obs_key_shapes, ac_dim, accelerator):
    """
        ...
        for k in self.optim_params:
            ...
            if k in self.nets:
                if isinstance(self.nets[k], nn.ModuleList):
                    self.optimizers[k] = [
                        self.accelerator.prepare(TorchUtils.optimizer_from_optim_params(net_optim_params=self.optim_params[k], net=self.nets[k][i]))
                        for i in range(len(self.nets[k]))
                    ]
                    self.lr_schedulers[k] = [
                        self.accelerator.prepare(TorchUtils.lr_scheduler_from_optim_params(net_optim_params=self.optim_params[k], net=self.nets[k][i], optimizer=self.optimizers[k][i]))
                        for i in range(len(self.nets[k]))
                    ]
                else:
                    self.optimizers[k] = self.accelerator.prepare(TorchUtils.optimizer_from_optim_params(
                        net_optim_params=self.optim_params[k], net=self.nets[k]))
                    self.lr_schedulers[k] = self.accelerator.prepare(TorchUtils.lr_scheduler_from_optim_params(
...
class RolloutPolicy(object):
    ...
    def __call__(self, ob, goal=None):
        ...
        ac = self.policy.get_action(obs_dict=ob)
