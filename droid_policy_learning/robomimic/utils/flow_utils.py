...
def track_to_xyz_flow(tracks, pointclouds):
    """
    tracks: (B, T, N, 2)
    pointclouds: (B, T, H, W, 3)
    
    return: (B, T, N, 3)
    """
    b, t, h, w, _ = pointclouds.shape
    _, _, n, _ = tracks.shape

    tracks[:, :, :, 0] = torch.clamp(tracks[:, :, :, 0] * w, 0, w-1)
    tracks[:, :, :, 1] = torch.clamp(tracks[:, :, :, 1] * h, 0, h-1)
    
    xyz_flow = np.zeros((b, t, n, 3))
    tracks = tracks.cpu()
    
    for i in range(b):
        for j in range(t):
            for k in range(n):
                xyz_flow[i, j, k] = pointclouds[i, j, int(tracks[i, j, k, 1]), int(tracks[i, j, k, 0])]
            
    return xyz_flow # (B, T, N, 3)

def draw_xyz_flow_on_pcd(xyz_flow : np.ndarray, pcd : np.ndarray):
    """
        xyz_flow (np.ndarray): (T, N1, 3)
        pcd (np.ndarray): (N2, 6)
    """
    color_map = cm.get_cmap("cool")
    ...
    traj_length = xyz_flow.shape[0]
    
    color = np.zeros((traj_length, 3))
    for s in range(traj_length):
        color[s] = np.array(color_map((s) / traj_length)[:3])
        
    for traj_idx in range(xyz_flow.shape[1]):
        traj = xyz_flow[:, traj_idx]
        traj = np.concatenate([traj, color], axis=-1) # (T, 6)
        result_pcd = np.concatenate([result_pcd, traj], axis=0)
        
    return result_pcd # (N1 + T * N2, 6)
