...
def load_data_for_training(config, obs_keys):
    ...
    if config.experiment.validate:
        ...
        valid_dataset = dataset_factory_for_val(config, obs_keys, filter_by_attribute=valid_filter_by_attribute)
    else:
        train_dataset = dataset_factory(config, obs_keys, filter_by_attribute=train_filter_by_attribute)
        valid_dataset = None
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    ds_kwargs["hdf5_path"] = [ds_cfg["path"] for ds_cfg in config.train.data if ds_cfg.get("train", True)]
    ds_kwargs["filter_by_attribute"] = [filter_by_attribute for ds_cfg in config.train.data if ds_cfg.get("train", True)]
    ds_weights = [ds_cfg.get("weight", 1.0) for ds_cfg in config.train.data if ds_cfg.get("train", True)]
    ds_labels = [ds_cfg.get("label", "dummy") for ds_cfg in config.train.data if ds_cfg.get("train", True)]
...
def dataset_factory_for_val(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    """
    Create a SequenceDataset instance to pass to a torch DataLoader.
    ...
    ds_kwargs["hdf5_path"] = [ds_cfg["path"] for ds_cfg in config.train.data if not ds_cfg.get("train", True)]
    ds_kwargs["filter_by_attribute"] = [filter_by_attribute for ds_cfg in config.train.data if not ds_cfg.get("train", True)]
    ds_weights = [ds_cfg.get("weight", 1.0) for ds_cfg in config.train.data if not ds_cfg.get("train", True)]
    ds_labels = [ds_cfg.get("label", "dummy") for ds_cfg in config.train.data if not ds_cfg.get("train", True)]
...
def save_model(model, config, env_meta, shape_meta, ckpt_path, accelerator, obs_normalization_stats=None, action_normalization_stats=None):
    """
    ...
    params = dict(
        model=unwarped_model.serialize(),
        config=config.dump(),
