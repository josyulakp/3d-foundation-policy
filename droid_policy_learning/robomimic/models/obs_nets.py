...
def obs_encoder_factory(
    ...
    for k, obs_shape in obs_shapes.items():
        ...
        if "pointcloud" in k:
            ObsUtils.register_encoder_core(PointcloudEncoder)
...
class ObservationEncoder(Module):
    ...
    def _create_layers(self):
        ...
        for k in self.obs_shapes:
            ...
            if "state" in k:
                self.state_shape += int(np.prod(feat_shape))
            elif "camera" in k:
                self.camera_shape = int(np.prod(feat_shape))
            elif "lang" in k:
                self.lang_shape += int(np.prod(feat_shape))
        
        self.state_emb = nn.Sequential(
            nn.Linear(self.state_shape, 512),
            nn.Mish(),
            nn.Linear(512, 1024),
        )
        
        self.camera_emb = nn.Linear(1024, 1024)
        
        self.lang_emb = nn.Linear(1024, 1024)
    ...
    def final_collation(self, feats):
        state_keys = [a for a in feats.keys() if "state" in a]
        camera_keys = [a for a in feats.keys() if "camera" in a]
        lang_keys = [a for a in feats.keys() if "lang" in a]
        # import pdb; pdb.set_trace()
        state_feats = torch.cat([TensorUtils.flatten(feats[k], begin_axis=1).unsqueeze(1) for k in state_keys], dim=-1)
        state_feats = self.state_emb(state_feats)
        
        # camera_feats = torch.cat([feats[k] for k in camera_keys], dim=1)
        camera_feats = torch.cat([TensorUtils.flatten(feats[k], begin_axis=1).unsqueeze(1) for k in camera_keys], dim=1)
        camera_feats = self.camera_emb(camera_feats)
        
        lang_feats = torch.cat([feats[k] for k in lang_keys], dim=1)
        lang_feats = self.lang_emb(lang_feats)        
        return OrderedDict({"state": state_feats, "camera": camera_feats, "lang": lang_feats})
