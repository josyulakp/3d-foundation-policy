...
def train(config, accelerator):
    ...
    if ds_format == "droid_rlds":
        raise NotImplementedError("RLDS dataset format not yet supported for training")
        ...
        # AcceleratorState().deepspeed_plugin.deepspeed_config['train_micro_batch_size_per_gpu'] = config.train.batch_size
    ...
    for epoch in range(1, config.train.num_epochs + 1): # epoch numbers start at 1
        ...
        #     with torch.no_grad():
        #         step_log, _ = TrainUtils.run_epoch(model=model, data_loader=valid_loader, epoch=epoch, validate=True, num_steps=valid_num_steps)
        #     if accelerator.is_main_process:
        #         for k, v in step_log.items():
        #             if k.startswith("Time_"):
...
def main(args):
    ...
    accelerator = Accelerator(kwargs_handlers=[InitProcessGroupKwargs(timeout = timedelta(seconds=3600))])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action=str,
        default=None,
        help="(optional) if provided, override the checkpoint defined in the config"
    )
